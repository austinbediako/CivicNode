 ...
 *
 * The executor must own the AdminCap for the proposal's community.
 */
export async function executeProposal(
  ...
  try {
    ...
    console.log(`[sui] Proposal ${proposalObjectId} executed. Digest: ${response.digest}`);
    return response.digest;
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    throw new Error(`Failed to execute proposal ${proposalObjectId} on-chain: ${message}`);
  }
}
